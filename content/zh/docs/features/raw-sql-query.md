...
由于最终 SQL 语句不可能被 Clusterpedia 封装、转义、参数化或验证，因此无论是启用 `AllowRawSQLQuery` 还是 `AllowParameterizedSQLQuery`  Feature Gate 都将可能引入 SQL 注入漏洞。
...
并且由于后来 `AllowParameterizedSQLQuery`  Feature Gate 的引入，该 Feature Gate 由于在防御 SQL 注入方面有很严重的缺陷，会在将来的版本中被废弃，不再建议使用这个 Feature Gate 。
